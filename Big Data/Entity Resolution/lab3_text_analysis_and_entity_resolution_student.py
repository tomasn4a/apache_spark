...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.sortBy(lambda (a,b): -len(b)).collect()
...
# TODO: Replace <FILL IN> with appropriate code
corpusRDD = googleRecToToken.fullOuterJoin(amazonRecToToken)
...
def norm(a):
    ...
    """
    return math.sqrt(reduce(lambda x,y: x+y, list(a.get(x)*a.get(x) for x in a.keys())))
...
avgSimDups = trueDupsRDD.map(lambda (a,(b,c)): b/float(trueDupsCount)).reduce(lambda a,b: a+b)

nonDupsRDD = (sims
              .leftOuterJoin(goldStandard)
              .filter(lambda (a,(b,c)): c==None))

nonDupsCount = nonDupsRDD.count()

avgSimNon = nonDupsRDD.map(lambda (a,(b,c)): b/float(nonDupsCount)).reduce(lambda a,b: a+b)
...
# TODO: Replace <FILL IN> with appropriate code
fullCorpusRDD = (google+amazon).map(lambda (a,b):(a,tokenize(b)))
