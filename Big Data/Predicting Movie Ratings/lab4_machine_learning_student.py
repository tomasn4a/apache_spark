...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    nRatings = len(list(IDandRatingsTuple[1]))
    averageRating = sum(list(IDandRatingsTuple[1]))/float(nRatings)
    return (IDandRatingsTuple[0],(nRatings,averageRating))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        ...
                        .map(lambda movie: math.pow(movie[1][1]-movie[1][0],2)))
    ...
    return float(math.sqrt(totalError/numRatings))
...
trainingAvgRating = trainingRDD.map(lambda movie: movie[2]).reduce(lambda a,b: a+b)/trainingRDD.count()
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda movie: (movie[0],movie[1],trainingAvgRating))
...
myRatedMovies = [(myUserID,551,3.8),(myUserID,1300,2.3),(myUserID,633,4.0),(myUserID,1218,4.3),
                 (myUserID,1047,4.45),(myUserID,1088,3.9),(myUserID,1039,3.97),
                 (myUserID,1438,2.8),(myUserID,993,3.6),(myUserID,983,2.1)
...
# TODO: Replace <FILL IN> with appropriate code
myList = []
for x in myRatedMovies:
    myList.append(myRatedMovies[1])
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda a: a[0] not in myList)).map(lambda a: (0,a[0]))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD)
                       .map(lambda a: (a[1][0][0],a[1][1],a[1][0][1]))).filter(lambda a: a[2]>75)
