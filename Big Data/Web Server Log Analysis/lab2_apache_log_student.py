...
not200 = access_logs.filter(lambda log: log.response_code!=200).map(lambda log: (log.endpoint,1))

endpointSum = not200.reduceByKey(lambda a,b: a+b)
...
#print dayToHostPairTuple.take(15)
#data = [(1, 'a'), (1, 'b'), (2, 'c'), (2, 'c'), (2, 'e'), (3, 'f')]
#rdd = sc.parallelize(data)
#result = rdd.groupByKey().map(lambda x : (x[0], len(list(set(list(x[1])))))).collect()
#print result
...
dayAndHostTuple = access_logs.map(lambda log: ((log.date_time.day,log.host),1)).reduceByKey(lambda a,b: a+b).map(lambda ((a,b),c): (a,c))

groupedByDay = dayAndHostTuple.reduceByKey(lambda a,b: a+b)

sortedByDay = groupedByDay.join(dailyHosts).map(lambda (a,(b,c)): (a,b/c)).sortByKey()
...
errDateSum = errDateCountPairTuple.map(lambda (a,b):(a,len(b)))
...
hourCountPairTuple = badRecords.map(lambda log: (log.date_time.hour,log.host)).groupByKey()

hourRecordsSum = hourCountPairTuple.map(lambda (a,b):(a,len(b)))
