...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    for feature in rawFeats:
        indices.append(OHEDict[feature])
    ones = [1.]*len(indices)
    return SparseVector(numOHEFeats, sorted(indices), ones)
...
def parsePoint(point):
    ...
    for i in range(len(pointSplit)-1):
        res.append((i,str(pointSplit[i+1])))
    return res
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    pointSplit = point.split(',')
    label = pointSplit[0]
    feat = oneHotEncoding(parsePoint(point),OHEDict,numOHEFeats)
...
def parseHashPoint(point, numBuckets):
    ...
    pointSplit = point.split(',')
    label = pointSplit[0]
    feats = SparseVector(numBuckets,hashFunction(numBuckets,parsePoint(point)))
...
# TODO: Replace <FILL IN> with appropriate code
def computeSparsity(data,n=0,d=0):
    ...
    """
    return data.map(lambda x: (float(len(x.features.indices)))/len(x.features)).mean()
