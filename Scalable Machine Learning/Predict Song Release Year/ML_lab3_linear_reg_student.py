...
def parsePoint(line):
    ...
    return LabeledPoint(floatList[0],floatList[1:])
...
averageTrainYear = (parsedTrainData
                    ...
                    .reduce(lambda x,y: x+y))/float(parsedTrainData.count())
...
def calcRMSE(labelsAndPreds):
    ...
    """
    return math.sqrt(labelsAndPreds.map(lambda i: squaredError(i[0],i[1])).reduce(lambda x,y: x+y)/labelsAndPreds.count())
...
def twoWayInteractions(lp):
    ...
    """
    interactions = itertools.product(lp.features,repeat=2)
    res = [i[0]*i[1] for i in list(interactions)]
    return LabeledPoint(lp.label,np.hstack((lp.features,res)))
